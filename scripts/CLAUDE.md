...
├── container-bootstrap.sh  # コンテナ初回展開プリミティブ（clone + install.sh --container。devcontainer 非依存）
...
### `install.sh --relink`（links-only 高速経路）

`--relink` は **Stow / `.config` リンクだけ**を張り直し、Homebrew / brew bundle / mise / Docker /
win32yank / Catppuccin clone / ni / VS Code 等の重処理を**全てスキップ**する。`--container` と併用する
（例: `install.sh --container --relink`）。コンテナの self-heal（`zsh/.zshrc`）が brew を走らせずに
symlink を復旧するために使う。フラグ無し / `--container` 単独の挙動は不変。

---

## container-bootstrap.sh

生 docker / docker compose（**devcontainer 非依存**）でコンテナに dotfiles を持ち込む初回展開プリミティブ。
`docker exec` 後に1コマンドで使う。冪等。

```bash
bash scripts/container-bootstrap.sh
curl -fsSL <raw>/scripts/container-bootstrap.sh | bash   # repo が無い環境（git 必須）
```

- `~/dotfiles` が無ければ clone（named volume 永続時は clone をスキップ）→ `install.sh --container` を exec。
- 上書き可能: `DOTFILES_REPO`（既定は zsh の `DOTFILES_REPO` と同じ libra-tech-japan/dotfiles）/ `DOTFILES_DIR`。
- 追加引数は `install.sh` へ素通し（例: `container-bootstrap.sh --relink`）。
- 自動復元（`docker exec` した瞬間）は `zsh/.zshrc` の self-heal が担当。bootstrap は**初回／手動の復旧**用。
  両者の使い分けと永続化（named volume）は README「2.2 コンテナへの持ち込み」を参照。
